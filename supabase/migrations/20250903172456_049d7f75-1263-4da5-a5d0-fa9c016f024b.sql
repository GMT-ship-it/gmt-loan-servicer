...
-- Trigger wrapper function to handle TG_OP logic
create or replace function public.handle_bbc_items_change()
returns trigger
language plpgsql
security definer
set search_path = public
...
declare
  v_report_id uuid;
begin
  -- determine which report_id to use based on operation
  ...
  -- recalculate the header totals
  perform public.recalc_bbc_header(v_report_id);
...
for each row execute function public.handle_bbc_items_change();