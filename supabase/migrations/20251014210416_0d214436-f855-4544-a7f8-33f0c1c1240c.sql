-- Step 16: Go-Live Checklist Implementation
...
-- RLS policies for loan-documents bucket (private, only lenders and document owners)
...
-- RLS policies for statements bucket (private, lenders + statement owners)
...
-- Additional indexes for covenant and facility lookups
...
-- 3) Verify non-accrual safety is already handled in run_daily_interest_accrual
-- The function already filters: where status = 'active' which excludes charged_off
-- Let's add an explicit non_accrual check to be extra safe
...
-- 4) Create a helper function to check RLS status
