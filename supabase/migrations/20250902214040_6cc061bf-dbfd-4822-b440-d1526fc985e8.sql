...
-- Borrowers: can view only their own facility's draw requests
drop policy if exists "draws_borrower_read" on public.draw_requests;
create policy "draws_borrower_read" on public.draw_requests
for select using (
...
-- Borrowers: can insert draw requests for their own facilities
drop policy if exists "draws_borrower_insert" on public.draw_requests;
create policy "draws_borrower_insert" on public.draw_requests
